...
az container create \
    ...
    --name wspsdemo-hello-world-cli \
    --dns-name-label wspsdemo-hello-world-cli \
    --image mcr.microsoft.com/azuredocs/aci-helloworld \
    --ports 80


#Show the container info
az container show --resource-group 'psdemo-rg' --name 'wspsdemo-hello-world-cli' 


#Retrieve the URL, the format is [name].[region].azurecontainer.io
URL=$(az container show --resource-group 'psdemo-rg' --name 'wspsdemo-hello-world-cli' --query ipAddress.fqdn | tr -d '"') 
echo "http://$URL"
...
ACR_NAME='wspsdemoacr' #<---change this to match your globally unique ACR Name
...
az container create \
    --resource-group psdemo-rg \
    --name wspsdemo-webapp-cli \
    --dns-name-label wspsdemo-webapp-cli \
    --ports 80 \
...
az container show --resource-group psdemo-rg --name wspsdemo-webapp-cli  


#Get the URL of the container running in ACI...
#This is our hello world app we build in the previous demo
URL=$(az container show --resource-group psdemo-rg --name wspsdemo-webapp-cli --query ipAddress.fqdn | tr -d '"') 
echo $URL
curl $URL


#Step 5 - Pull the logs from the container
az container logs --resource-group psdemo-rg --name wspsdemo-webapp-cli
